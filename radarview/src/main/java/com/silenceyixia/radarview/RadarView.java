...
/**
 * Created by zwzd-01 on 2017/5/31.
 */
...
public class RadarView extends View {
    ...
    public RadarView(Context context, AttributeSet attrs) {
        ...
        mTextSize = (int) ta.getDimension(R.styleable.RadarView_rvTextSize, 15);
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        maxRadius = (int) (Math.min(width >> 1, maxRadius) * 0.9);
        //网间距
        dalta = maxRadius / 5;
        postInvalidate();
    ...
    private void drawText(Canvas canvas) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (tempX > centerX) {
                if (tempY < centerY) {
                    canvas.drawText(elementBean.getTitle(), tempX - (rect.width() >> 1), tempY - 5, mTextPaint);
                } else if (tempY > centerY) {
                    canvas.drawText(elementBean.getTitle(), tempX - (rect.width() >> 1), tempY + 5 + rect.height(), mTextPaint);
                } else if (tempY > centerY - 1 && tempY < centerY + 1) {
                    canvas.drawText(elementBean.getTitle(), tempX + 5, tempY + (rect.height() >> 1), mTextPaint);
                }
            } else {
                if (tempY < centerY) {
                    canvas.drawText(elementBean.getTitle(), tempX - (rect.width() >> 1), tempY - 5, mTextPaint);
                    ...
                    canvas.drawText(elementBean.getTitle(), tempX - 5 - rect.width(), tempY + (rect.height() >> 1), mTextPaint);
                }
            }
        }
    }
